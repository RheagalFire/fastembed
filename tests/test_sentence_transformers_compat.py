...
# Attempt to import SentenceTransformer and set a flag based on the success of the import
try:
...
# Attempt to import BetterTransformer and set a flag based on the success of the import
try:
...
# Skip the test if sentence-transformers is not installed
...
def test_performance_vs_sentence_transformers(model):
    ...
    time.sleep(1)
    t2 = timeit.timeit(lambda: model_st.encode(sample_sentence, batch_size=batch_size, normalize_embeddings=True), number=10)
